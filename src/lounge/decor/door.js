...
function buildFrame() {
  ...
  frame.add(
    slab(jambWidth, DOOR_HEIGHT + jambWidth, FRAME_DEPTH, materials.brass).translateX(
  ...
  frame.add(
    slab(DOOR_WIDTH + jambWidth * 2, jambWidth, FRAME_DEPTH, materials.brass).translateY(
      DOOR_HEIGHT / 2 + jambWidth / 2,
    ),
  )
...
function buildLeaf() {
  ...
  for (const side of [-1, 1]) {
    const upperPanel = slab((DOOR_WIDTH - 0.08 - panelInset * 3) / 2, 0.85, 0.015, panelMat)
...
export function addDoor(scene) {
  ...
  door.position.set(
    0,
    DOOR_HEIGHT / 2,
    ROOM.depth / 2 - WALL_THICKNESS / 2 - LEAF_THICKNESS / 2 - 0.005,
  )
