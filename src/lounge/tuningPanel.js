...
export function createTuningPanel({ scene, renderer, bloom, setQuality, smoke, stageSpots }) {
  ...
  if (stageSpots) {
    ...
    focoFolder
      .add(params, 'focoIntensidad', 0, 2, 0.05)
      .name('intensidad')
      .onChange(applyMultiplier('foco'))
    focoFolder
      .add(params, 'focoAnchura', 0.5, 2.5, 0.05)
      .name('anchura')
      .onChange((v) => {
        stageSpots.setWidth(v)
      })
    focoFolder
      .add(params, 'focoHaz', 0, 0.15, 0.005)
      .name('haz')
      .onChange((v) => {
        stageSpots.beamMaterial.opacity = v
      })
    focoFolder
      .add(params, 'focoLente', 0, 4, 0.1)
      .name('lente')
      .onChange((v) => {
  ...
  if (smoke) {
    ...
    humoFolder
      .add(params, 'humoOpacidad', 0, 0.2, 0.005)
      .name('opacidad')
      .onChange((v) => {
        smoke.opacity.value = v
      })
    humoFolder
      .add(params, 'humoTamano', 0.5, 5, 0.05)
      .name('tamaño')
      .onChange((v) => {
